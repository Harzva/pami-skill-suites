# Elsevier Skills v2.7.0（中文）

面向 Elsevier 期刊投稿的上下文安全型 Journal Skill-Suite。本仓库为非官方项目，不隶属于 Elsevier。
...
## v2.4.0 候选证据卡与 Metadata Connector 计划

本版本为主图网站 / RAG workflow 增加候选证据卡系统。每个候选论文都会区分 DOI 证据、出版社页面证据、机构仓储 OA 证据、license 证据、permission 证据、第三方素材证据和人工复核备注。v2.4.0 仍然是离线安全构建，不声称已经完成 Crossref / OpenAlex / Unpaywall / 出版社页面的 live lookup。

在 extraction gate 解锁之前，禁止新增抽图、公开展示或复用任何主图资产。

## v2.4.0 Evidence-Gated OA Candidate Promotion

v2.4.0 增加了 evidence-gated candidate promotion、connector simulation、candidate evidence diff、human reviewer decision log、extraction-ready candidates 和 metadata-only RAG-ready candidates。默认 `skills/` 仍然保持 context-safe，不暴露 `skills-advanced/`、`expansion_packs/`、`presets/` 或 `main_figure_site/`。

本版本仍然是 offline-safe release build：不会声称已经完成 Crossref / OpenAlex / Unpaywall / 出版商页面 / 机构仓储的实时核验，也不会允许新的主图抽取。真实复用或公开展示前必须完成 DOI、OA 来源、license、permission、第三方素材和人工审查。
